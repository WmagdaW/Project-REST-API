...
class TrelloFacadeTest {
    ...
    public void shouldCreateCreatedTrelloCardDto() {
        ...
        TrelloCard trelloCard = new TrelloCard("Card1", "1", "1", "1");
        TrelloCardDto trelloCardDto = new TrelloCardDto("Card1", "1", "1", "1");

        when(trelloMapper.mapToCardDto(trelloCard)).thenReturn(trelloCardDto);
        when(trelloMapper.mapToCard(trelloCardDto)).thenReturn(trelloCard);
        when(trelloService.createTrelloCard(trelloCardDto)).thenReturn(theCard);

        //When
        CreatedTrelloCardDto testCard = trelloFacade.createCard(trelloCardDto);

        //Then
        assertThat(testCard.getId()).isEqualTo("1");
        assertThat(testCard.getName()).isEqualTo("Test card");
        assertThat(testCard.getShortUrl()).isEqualTo("url");
        assertThat(testCard.getBadges().getVotes()).isEqualTo(1);
        assertEquals(1, testCard.getBadges().getAttachments().getTrello().getBoard());
        assertEquals(2, testCard.getBadges().getAttachments().getTrello().getCard());
        assertEquals(1, testCard.getBadges().getVotes());
